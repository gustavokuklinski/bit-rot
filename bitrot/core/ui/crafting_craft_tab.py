    ...
    def filter_recipes(self, recipes, search_text):
        ...
        for r in recipes:
            ...
            if search_text:
                ...
                matches_output = (st in r.output_name.lower()) or (st in tr('item', r.output_name).lower())
       ...
       # Ingredients list
        ...
        if r.req_level:
            ...
            for idx, (attr, lvl) in enumerate(items):
                ...
                s_surf = font_12.render(txt, False, s_color)
        ...
        if self.modal.warning_message:
            warn_surf = font_12.render(self.modal.warning_message, False, RED)
            element_cursor_y -= 20
