...
def create_item_from_name(cls, item_name, randomize_durability=False, force_color=None, spawn_loot=True):
    ...
    barricade_health = template.get('health_max')
    ...
    new_item = cls(
        ...
        slot=slot, defence=defence, speed=speed, state=state, min_light=min_light, 
        max_light=max_light, fuel_type=fuel_type, text=text, min_reduce=min_reduce, 
        max_reduce=max_reduce, sounds=sounds, attribute_modifiers=attribute_modifiers, 
        status_effect=status_effect, effects=effects, repair_list=repair_list, 
        knockback=knockback, machine_gun=machine_gun, firing_second=firing_second, 
        allow_sleep=allow_sleep, key_id=key_id, firing_distance=firing_distance, 
