...
# --- NEW HELPER FUNCTION ---
def _find_spawn_spot_near(initial_pos_px, occupied_tiles, max_radius=5):
    """
...
def spawn_initial_zombies(obstacles, zombie_spawns, items_on_ground, limit=1000, spawns_per_marker=None):
    zombies = []
    ...
    for pos in zombie_spawns: # Loop 1: Over each 'Z' marker (e.g., 50 markers)
        ...
        for _ in range(spawns_per_marker): # Loop 2: N zombies per marker (e.g., 3)
            ...
            spawn_spot_px = _find_spawn_spot_near(pos, occupied_tiles)
