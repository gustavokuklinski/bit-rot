...
class BaseModal:
    ...
    def get_modal_dimensions(self):
        if self.modal['type'] == 'inventory': return INVENTORY_MODAL_WIDTH, INVENTORY_MODAL_HEIGHT
        elif self.modal['type'] == 'status': return STATUS_MODAL_WIDTH, STATUS_MODAL_HEIGHT
        elif self.modal['type'] == 'container': return CONTAINER_MODAL_WIDTH, CONTAINER_MODAL_HEIGHT
        elif self.modal['type'] == 'nearby': return NEARBY_MODAL_WIDTH, NEARBY_MODAL_HEIGHT
        elif self.modal['type'] == 'messages': return MESSAGES_MODAL_WIDTH, MESSAGES_MODAL_HEIGHT
        elif self.modal['type'] == 'text': return TEXT_MODAL_WIDTH, TEXT_MODAL_HEIGHT
        elif self.modal['type'] == 'gear': return GEAR_MODAL_WIDTH, GEAR_MODAL_HEIGHT
        elif self.modal['type'] == 'mobile': return MOBILE_MODAL_WIDTH, MOBILE_MODAL_HEIGHT
        elif self.modal['type'] == 'vehicle': return VEHICLE_MODAL_WIDTH, VEHICLE_MODAL_HEIGHT
        elif self.modal['type'] == 'crafting': return CRAFTING_MODAL_WIDTH, CRAFTING_MODAL_HEIGHT
        elif self.modal['type'] == 'big_map': return MAP_MODAL_WIDTH, MAP_MODAL_HEIGHT
        elif self.modal['type'] == 'help': return HELP_MODAL_WIDTH, HELP_MODAL_HEIGHT
        elif self.modal['type'] == 'npc_dialog': return NPC_DIALOG_MODAL_WIDTH, NPC_DIALOG_MODAL_HEIGHT
        elif self.modal['type'] == 'slots': return SLOTS_MODAL_WIDTH, SLOTS_MODAL_HEIGHT
        return 244, 240
    ...
    def get_buttons(self):
        ...
        # [FIX]: Use `.get('id')` to ensure missing IDs do not throw an error and freeze execution
